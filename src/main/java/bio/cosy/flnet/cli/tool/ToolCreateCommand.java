...
import bio.cosy.flnet.cli.base.ToolType;
...
/** Asks for the settings of a new tool and fills a {@link FLNetTool}; {@link FLNetToolBO} generates it. */
@Command(name = "create",
...
public class ToolCreateCommand implements Callable<Integer> {
    ...
    public Integer call() {
        ...
        Ui.success("Created " + tool.getToolType().cliName() + (tool.isFederated() ? " (federated)" : "") + " tool '"
                + tool.getName() + "' in " + tool.getDirectory());
        Ui.info("Next steps:");
        Path relative = Path.of("").toAbsolutePath().relativize(tool.getDirectory());
        Ui.command("cd " + (relative.toString().isEmpty() ? "." : relative));
        Ui.command("python3 -m venv .venv && source .venv/bin/activate");
        Ui.command("pip install -r requirements.txt");
        if (toolBO.needsAppId(tool)) {
            Ui.info("Set APP_ID in .env to the id of your tool on the platform, then run:");
        }
        Ui.command("python main.py");
        return 0;
    }
