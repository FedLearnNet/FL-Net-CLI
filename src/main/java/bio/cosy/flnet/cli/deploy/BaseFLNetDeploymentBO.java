...
/**
 * Business logic shared by platforms and clients: every instance is a directory
 * {@code <flnet.home>/<kind>s/<name>} (deployments elsewhere are linked there), so listing is a
 * directory scan and loading reads the instance's .env and secret files into its model.
 */
public abstract class BaseFLNetDeploymentBO<T extends BaseFLNetDeployableInstance> {
    ...
    /** A new, unsaved deployment with the configured defaults. */
    protected abstract T create();

    /** Compose project name of the instance named {@value BaseFLNetDeployableInstance#DEFAULT_NAME}. */
    protected abstract String baseProjectName();

    /** Hook after loading, e.g. to derive values that are not stored. */
    protected void afterLoad(T instance) {
    }

    // ---------------------------------------------------------------- locations

    /** Root of all instances: {@code flnet.home} ({@code $FLNET_HOME}) or {@code ~/fl-net}. */
    public Path home() {
    ...
    /**
     * Compose project of a new instance, which scopes containers, volumes and networks: the
     * configured name for the default instance, otherwise suffixed with the instance name.
     */
    ...
    /** A new deployment named {@code name} in {@code directory}. */
    ...
    /** Reads an existing deployment; its stored name wins over {@code fallbackName}. */
    ...
    /** All initialized instances, sorted by name. */
    public List<T> list() {
    ...
    /**
     * The deployment an operation ({@code up}, {@code logs}, ...) works on: explicit {@code --dir}
     * or {@code --name}, else the only one, else ask (interactive) or fail with the list of names.
     */
    ...
    /**
     * The deployment {@code init} creates or reconfigures: an existing one is loaded with all its
     * settings (they become the defaults), otherwise a new one with the configured defaults.
     * Without {@code --name} the first instance is {@value BaseFLNetDeployableInstance#DEFAULT_NAME};
     * once instances exist they are shown and a name is required.
     */
    public T prepareForInit(InstanceOptions options, Prompter prompter) {
        ...
        if (name == null && options.dir != null) {
            ...
            name = EnvFile.read(options.dir.resolve(".env")).get(BaseFLNetDeployableInstance.NAME_VARIABLE);
        }
        if (name == null) {
        ...
        if (existing.isEmpty() && !all.isEmpty()) {
            Ui.info("Setting up an additional " + kind().displayName() + " named '" + name + "'.");
        }
    ...
    private String askName(List<T> all, Prompter prompter) {
        ...
        Ui.info("This machine already has " + all.size() + " " + kind().displayName() + " instance" + (all.size() == 1 ? "" : "s") + ":");
        all.forEach(i -> Ui.info("  " + summary(i)));
        ...
        Ui.info("Enter an existing name to reconfigure it, or a new name to set up another " + kind().bundle() + ".");
        return prompter.text("--name", null, "Name", null, BaseFLNetDeployableInstance::validateName);
    }

    // ---------------------------------------------------------------- ports

    /** Plans ports of {@code instance} around every other deployment on this machine. */
    ...
    /**
     * Writes the deployment: static files from the bundle, secret files, .env and the link into the
     * instances home. Validates first, so nothing is written for an invalid configuration.
     */
    public Bundle.InstallResult save(T instance, boolean refreshFiles, Path bundleDir) {
        instance.requireValid(validator);
        Bundle.InstallResult files = Bundle.install(kind(), instance.getDirectory(), bundleDir, refreshFiles);
        if (!BaseFLNetDeployableInstance.DEFAULT_NAME.equals(instance.getName())) {
            Bundle.requireScopedContainerNames(instance);
        }
        instance.toSecretFiles().forEach((file, variables) -> EnvFile.write(instance.getSecretsDirectory().resolve(file), variables));
        EnvFile.write(instance.getEnvFile(), instance.toEnv(), instance.envComments());
        afterSave(instance);
        register(instance);
        return files;
    }

    /** Hook after the files are written, e.g. to patch configs. */
    protected void afterSave(T instance) {
    }

    /** Whether any secret file exists; with {@link BaseFLNetDeployableInstance#hasSecrets()} it tells complete from partial. */
    ...
    /** Makes a deployment outside the instances home visible to {@code list} with a symbolic link. */
    public void register(T instance) {
        ...
        try {
            ...
            Ui.warn("Could not link " + instance.getDirectory() + " into " + link + " (" + e.getMessage()
                    + "); use --dir to manage this " + kind().bundle() + ".");
        }
    }

    /**
     * Deletes the deployment's directory (configuration, secrets, certificates generated by flnet)
     * and its link in the instances home. Docker resources are removed separately.
     */
    ...
    /**
     * {@code " --name x"} when commands need the name to find this deployment, i.e. when it is not
     * the only one, otherwise empty.
     */
    ...
    /** "b  http://localhost:8251  ports 8251" */
