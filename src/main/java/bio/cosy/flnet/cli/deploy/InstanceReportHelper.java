...
public final class InstanceReportHelper {

    private InstanceReportHelper() {
    }
    ...
    public static void details(ComposeBO composeBO, BaseFLNetDeployableInstance instance, String nameFlag) {
        ConsoleHelper.heading(instance.getKind().displayName() + " '" + instance.getName() + "'");
        ...
        ConsoleHelper.blank();
        ConsoleHelper.info("Manage it with:");
        ConsoleHelper.command("flnet " + instance.getKind().bundle() + " up|down|pull|stop|restart|status|logs|compose|clean" + nameFlag);
        ConsoleHelper.command("flnet " + instance.getKind().bundle() + " init" + (nameFlag.isEmpty() ? " --name " + instance.getName() : nameFlag)
                + "    (reconfigure)");
    }

    public static void env(BaseFLNetDeployableInstance instance) {
        ConsoleHelper.heading(".env of " + instance.getKind().displayName() + " '" + instance.getName() + "' (" + instance.getEnvFile() + ")");
        Map<String, String> values = EnvFileHelper.read(instance.getEnvFile());
